  ...
  methods: {
    ...
    getTickValues(tl) {
      ...
      if (startYear % tl.tickInterval != 0 && notOK) {
        ...
        if (startYear - firstTick < 3) {startYear += tl.tickInterval}
      }
      const numTicks = Math.floor((tl.stopYear - startYear)/tl.tickInterval) + 1
      const tickValues = Array(numTicks).fill(startYear).map(
              (start, index) => start + (index * tl.tickInterval))
      if (firstTick) tickValues.unshift(firstTick)
      // if stopYear is not on a boundary, add it;
      if (!Number.isInteger((tl.stopYear - startYear)/tl.tickInterval)) { tickValues.push(tl.stopYear) }
      return tickValues
    },
    getTimeAxisSVGFn(tl) {
      // tl.timeScaleFn must already be assigned;
      return d3.axisBottom(tl.timeScaleFn)
